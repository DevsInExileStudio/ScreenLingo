...
## Autoload `PluginBridge`: the single access point to the native Android plugin.
## The UI talks only to this node — typed signals and methods — while on desktop
## (no plugin present) the settings live in memory so the interface can be debugged in the editor.
...
## Name of the native singleton in the Android build (Engine.get_singleton).
const PLUGIN_NAME := "YOUR_ID_HERE"

## Engine identifiers — the same ones used in the plugin's TranslationEngines.kt.
...
## Fallback values for desktop, where the plugin does not exist.
var _fallback := {
	"target": "en",
	"script": "latin",
	"engine": ENGINE_MLKIT,
...
## Returns false if the "Display over other apps" permission has to be granted first
## (the plugin opens the settings and starts the overlay itself once the user comes back).
...
## The engine key goes only into the plugin's private storage; it is never read back.
...
## LLM engine model; an empty string means the default (the engine registry lives in the UI layer).
...
## Floating button scale (0.75–1.5 of the base 54dp).
